...
if __name__ == '__main__':
    ...
    '''Getting prediction probabilties from baser learner two (KNN in this case) for training data'''

    prediction_probabilities2 = Knn_kFold_probability.training_input_base_model(train_reports, train_labels)

    prediction_probabilities_knn = []
    for x in prediction_probabilities2:
        for y in x:
            prediction_probabilities_knn.append(y)



    '''combining predictions get from base learners(svm + cs) which will be used for training meta learner'''

    # i = 0
    # trainDoc_probability = []
    # for x in prediction_probabilities:
    #     trainDoc_probability.append(np.append(x, cs_trainDoc[i]))
    #     i += 1

    '''combining predictions get from base learners(svm + knn) which will be used for training meta learner'''

    i = 0
    trainDoc_probability = []
    for x in prediction_probabilities:
        trainDoc_probability.append(np.append(x, prediction_probabilities_knn[i]))
        i += 1
    ...
    '''predictions get from base learner 2 for(cs) test data(original document)'''

    # cs_testDoc = Cosine_similarity.cos_sim(test_reports)

    '''predictions get from base learner 2(knn) for test data(original document)'''

    test_prediction_probability_knn = KNN.KNN_prob(trainDoc=train_reports, trainClass=train_labels,
                                                                testDoc=test_reports, testClass=test_labels, tfIdf=True)




    '''combining them(svm+cs)'''

    # i = 0
    # testDoc_probability = []
    # for x in test_prediction_probability:
    #     testDoc_probability.append(np.append(x, cs_testDoc[i]))
    #     i += 1

    '''combining them(svm+knn)'''

    i = 0
    testDoc_probability = []
    for x in test_prediction_probability:
        testDoc_probability.append(np.append(x, test_prediction_probability_knn[i]))
        i += 1
